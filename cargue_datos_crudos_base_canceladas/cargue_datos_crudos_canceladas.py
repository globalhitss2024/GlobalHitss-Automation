...
* OBJETIVO: Data Cruda base canceladas                                                *                                                                            *
* TABLA DE INGESTA POSTGRESQL: tbl_dados_crudos_canceladas                            *
...
fuentes = []
cantidad_registros = []
destino = [par.destino_canceladas]
estado = []
funcion_error = []
descripcion_error = []
...
def cargueResumen(id_ejecucion, fecha_inicio_date,fuentes,cantidad_registros,destino,estado):
    ...
    try:
        ...
        fuentes.append(par.nombre_archivo_canceladas)
...
def insertarErroresDB():
    ...
    try:
        ...
        cargueResumen(id_ejecucion_en_curso, fecha_inicio_tr,par.nombre_archivo_canceladas,0,par.destino_canceladas,2) 
        salidaLogMonitoreo()
...
# %%
def importarCanceladasExcel(ruta, nombre_archivo, hoja_calculo):
    """
    ...
    try:
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(importarCanceladasExcel.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def consultarCanceladasHistorico():
    """
    ...
    try:
        ...
        sql_consulta = """
        SELECT *
        FROM fuentes_cruda.tb_datos_crudos_canceladas
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(consultarCanceladasHistorico.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def limpiezaCamposCanceladas(base, fecha_inicio_date, id_ejecucion):
    """
    ...
    try:
        ...
        df_historico = consultarCanceladasHistorico()
        ...
        df_nuevos['fuente'] = 'CANCELADAS CORREOS'
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(limpiezaCamposCanceladas.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
def cargueDatosBD(df_final):
    ...
    try:
        ...
        nombre_tabla = 'tb_datos_crudos_canceladas'
...
if __name__ == "__main__":
    ...
    try:
        ...
        archivos = [f for f in os.listdir(par.ruta_fuente_canceladas) if archivo_modificado_hoy(os.path.join(par.ruta_fuente_canceladas, f))]
        archivos_actualizados = archivos
        #df_canceladas_historico = consultarCanceladasHistorico()


        if par.nombre_archivo_canceladas in archivos_actualizados:  
            id_ejecucion = generate_uuid().upper()
            id_ejecucion_en_curso = id_ejecucion
            base_excel_canceladas = importarCanceladasExcel(par.ruta_fuente_canceladas, par.nombre_archivo_canceladas, par.nombre_hoja_canceladas)
            df_canceladas_nuevo = limpiezaCamposCanceladas(base_excel_canceladas,fecha_inicio_date,id_ejecucion)
            #df_canceladas_nuevo = pd.merge(df_limpieza_campos_canceladas, df_canceladas_historico, left_on='ot', right_on= ['ot'], how='outer', indicator=True, suffixes=('_canceladas', '_historico')).query('_merge == "left_only"')
            #columnas_seleccionadas = [par.mapeo_columnas_canceladas[col] for col in par.mapeo_columnas_canceladas.keys() if par.mapeo_columnas_canceladas[col] in df_canceladas_nuevo.columns]
            #df_canceladas_nuevo = df_canceladas_nuevo[columnas_seleccionadas]
            #df_canceladas_nuevo.columns = df_canceladas_nuevo.columns.str.replace('_canceladas', '')
            fuentes.append(par.nombre_archivo_canceladas)
            registros = len(df_canceladas_nuevo)
            cantidad_registros.append(registros)
            
            if registros > 0:
                df_resumen = cargueResumen(id_ejecucion, fecha_inicio_date, par.nombre_archivo_canceladas,registros,par.destino_canceladas,1)
                cargueDatosBD(df_canceladas_nuevo)
