...
* OBJETIVO: Data Cruda base instaladas                                                *                                                                            *
* TABLA DE INGESTA POSTGRESQL: tbl_dados_crudos_instaladas                            *
...
def insertarErroresDB():
    ...
    try:
        ...
        descripcion_error.append(str(e)[:300])
        salidaLogMonitoreo()

# %%
def importarInstaladasExcel(ruta, nombre_archivo, hoja_calculo):
    """
    ...
    try:
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(importarInstaladasExcel.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def consultarInstaladasHistorico():
    """
    ...
    try:
        ...
        sql_consulta = """
        SELECT *
        FROM fuentes_cruda.tb_datos_crudos_instaladas
        WHERE valor_servicio > 0
        """
        df_instaladas_historico = pd.read_sql(sql_consulta, engine)
        #df_instaladas_historico = df_instaladas_historico.drop_duplicates(subset=['ot',])
    
        return df_instaladas_historico
...
# %%
def limpiezaCamposInstaladas(base, fecha_inicio_date, id_ejecucion):
    """
    ...
    try:
        ...
        df_historico = consultarInstaladasHistorico()
        ...
        df_nuevos['fuente'] = 'INSTALADAS CORREOS'
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(limpiezaCamposInstaladas.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
def cargueDatosBD(df_final):
    ...
    try:
        ...
        nombre_tabla = 'tb_datos_crudos_instaladas'
...
def configurarLogging():
    ...
    log_file = os.path.join(log_directory, "cargue_datos_crudos_canceladas.log")
...
if __name__ == "__main__":
    ...
    try:
        ...
        archivos = [f for f in os.listdir(par.ruta_fuente_instaladas) if archivo_modificado_hoy(os.path.join(par.ruta_fuente_instaladas, f))]
        archivos_actualizados = archivos
        #df_instaladas_historico = consultarInstaladasHistorico()


        if par.nombre_archivo_instaladas in archivos_actualizados:  
            id_ejecucion = generate_uuid().upper()
            id_ejecucion_en_curso = id_ejecucion
            base_excel_instaladas = importarInstaladasExcel(par.ruta_fuente_instaladas, par.nombre_archivo_instaladas, par.nombre_hoja_instaladas)
            df_instaladas_nuevo = limpiezaCamposInstaladas(base_excel_instaladas,fecha_inicio_date,id_ejecucion)
            #df_instaladas_nuevo = pd.merge(df_limpieza_campos_instaladas, df_instaladas_historico, left_on='ot', right_on= ['ot'], how='outer', indicator=True, suffixes=('_instaladas', '_historico')).query('_merge == "left_only"')
            #columnas_seleccionadas = [par.mapeo_columnas_instaladas[col] for col in par.mapeo_columnas_instaladas.keys() if par.mapeo_columnas_instaladas[col] in df_instaladas_nuevo.columns]
            #df_instaladas_nuevo = df_instaladas_nuevo[columnas_seleccionadas]
            #df_instaladas_nuevo.columns = df_instaladas_nuevo.columns.str.replace('_instaladas', '')
            fuentes.append(par.nombre_archivo_instaladas)
            registros = len(df_instaladas_nuevo)
            cantidad_registros.append(registros)
            
            if registros > 0:
                df_resumen = cargueResumen(id_ejecucion, fecha_inicio_date, par.nombre_archivo_instaladas,registros,par.destino_instaladas,1)
                cargueDatosBD(df_instaladas_nuevo)
