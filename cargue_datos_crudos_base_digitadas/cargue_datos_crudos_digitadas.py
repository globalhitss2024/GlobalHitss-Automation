...
* OBJETIVO: Data Cruda base digitadas                                                 *                                                                            *
* TABLA DE INGESTA POSTGRESQL: tbl_dados_crudos_digitadas                             *
...
def cargueResumen(id_ejecucion, fecha_inicio_date,fuentes,cantidad_registros,destino,estado):
    ...
    try:
        ...
        fuentes.append(par.nombre_archivo_digitadas)
...
def insertarErroresDB():
    ...
    try:
        ...
        cargueResumen(id_ejecucion_en_curso, fecha_inicio_tr,par.nombre_archivo_digitadas,0,par.destino_digitadas,2) 
        salidaLogMonitoreo()
...
# %%
def importarDigitadasExcel(ruta, nombre_archivo, hoja_calculo):
    """
    ...
    try:
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(importarDigitadasExcel.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def consultarDigitadasHistorico():
    """
    Función que consulta los datos historicos existentes en la base de datos de la tabla de tb_datos_crudos_digitadas
    ...
    try:
        ...
        SELECT *
        FROM fuentes_cruda.tb_datos_crudos_digitadas
        WHERE valor_servicio > 0
        """
        df_digitadas_historico = pd.read_sql(sql_consulta, engine)
        #df_digitadas_historico = df_digitadas_historico.drop_duplicates(subset=['ot',])

        return df_digitadas_historico
        
    except Exception as e:
        fuentes.append(par.nombre_archivo_digitadas)
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(consultarDigitadasHistorico.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def limpiezaCamposDigitadas(base, fecha_inicio_date, id_ejecucion):
    """
    ...
    try:
        ...
        df_base.fillna({
            ...
            'grupo_servicio': 'NO REGISTRA'
        }, inplace=True)
       
        df_historico = consultarDigitadasHistorico()
        ...
        df_nuevos['fecha_creacion'] = df_nuevos['fecha_creacion'].fillna(datetime(1899, 12, 30))
        ...
        df_nuevos['fuente'] = 'DIGITADAS CORREOS'
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(limpiezaCamposDigitadas.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
def cargueDatosBD(df_final):
    ...
    try:
        ...
        nombre_tabla = 'tb_datos_crudos_digitadas'
...
def configurarLogging():
    ...
    log_file = os.path.join(log_directory, "cargue_datos_crudos_digitadas.log")
...
if __name__ == "__main__":
    ...
    try:
        ...
        archivos = [f for f in os.listdir(par.ruta_fuente_digitadas) if archivo_modificado_hoy(os.path.join(par.ruta_fuente_digitadas, f))]
        archivos_actualizados = archivos
        #df_digitadas_historico = consultarDigitadasHistorico()


        if 'DIGITADAS.xls' in archivos_actualizados:  
            id_ejecucion = generate_uuid().upper()
            id_ejecucion_en_curso = id_ejecucion
            base_excel_digitadas = importarDigitadasExcel(par.ruta_fuente_digitadas, par.nombre_archivo_digitadas, par.nombre_hoja_digitadas)
            df_digitadas_nuevo = limpiezaCamposDigitadas(base_excel_digitadas,fecha_inicio_date,id_ejecucion)
            #df_digitadas_nuevo = pd.merge(df_limpieza_campos_digitadas, df_digitadas_historico, left_on='ot', right_on= ['ot'], how='outer', indicator=True, suffixes=('_digitadas', '_historico')).query('_merge == "left_only"')
            #columnas_seleccionadas = [par.mapeo_columnas_digitadas[col] for col in par.mapeo_columnas_digitadas.keys() if par.mapeo_columnas_digitadas[col] in df_digitadas_nuevo.columns]
            #df_digitadas_nuevo = df_digitadas_nuevo[columnas_seleccionadas]
            #df_digitadas_nuevo.columns = df_digitadas_nuevo.columns.str.replace('_digitadas', '')
            fuentes.append(par.nombre_archivo_digitadas)
            registros = len(df_digitadas_nuevo)
            cantidad_registros.append(registros)
            
            if registros > 0:
                df_resumen = cargueResumen(id_ejecucion, fecha_inicio_date, par.nombre_archivo_digitadas,registros,par.destino_digitadas,1)
                cargueDatosBD(df_digitadas_nuevo)
