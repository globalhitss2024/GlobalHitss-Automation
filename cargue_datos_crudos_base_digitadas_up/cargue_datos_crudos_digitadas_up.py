...
* OBJETIVO: Data Cruda base digitadas up                                              *                                                                            *
* TABLA DE INGESTA POSTGRESQL: tbl_crudo_digitadas_up                                 *
* FECHA CREACION: 31 de mayo de 2024                                                  *
...
fuentes = []
cantidad_registros = []
destino = [par.destino_digitadas_up]
estado = []
funcion_error = []
descripcion_error = []
...
def cargueResumen(id_ejecucion, fecha_inicio_date,fuentes,cantidad_registros,destino,estado):
    ...
    try:
        ...
        fuentes.append(par.nombre_archivo_digitadas_up)
...
def insertarErroresDB():
    ...
    try:
        ...
        cargueResumen(id_ejecucion_en_curso, fecha_inicio_tr,par.nombre_archivo_digitadas_up,0,par.destino_digitadas_up,2) 
        salidaLogMonitoreo()
...
# %%
def importarDigitadasUpCsv(ruta, nombre_archivo):
    """
    ...
    try:
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(importarDigitadasUpCsv.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def consultarDigitadasUpHistorico():
    """
    Función que consulta los datos historicos existentes en la base de datos de la tabla de tb_datos_crudos_digitadas_up
    ...
    try:
        ...
        sql_consulta = """
        SELECT *
        FROM fuentes_cruda.tb_datos_crudos_digitadas_up
        WHERE monthly_rental_charge > 0
        """
        df_digitadas_up_historico = pd.read_sql(sql_consulta, engine)
        #df_digitadas_up_historico = df_digitadas_up_historico.drop_duplicates(subset=['ot',])
    
        return df_digitadas_up_historico
        
    except Exception as e:
        fuentes.append(par.nombre_archivo_digitadas_up)
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(consultarDigitadasUpHistorico.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def limpiezaCamposDigitadasUp(base, fecha_inicio_date, id_ejecucion):
    """
    ...
    try:
        ...
        df_historico = consultarDigitadasUpHistorico()
        ...
        df_nuevos['fuente'] = 'DIGITADAS UP CORREOS'
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(limpiezaCamposDigitadasUp.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
def cargueDatosBD(df_final):
    ...
    try:
        ...
        nombre_tabla = 'tb_datos_crudos_digitadas_up'
...
def configurarLogging():
    ...
    log_file = os.path.join(log_directory, "cargue_datos_crudos_digitadas_up.log")
...
if __name__ == "__main__":
    ...
    try:
        ...
        archivos = [f for f in os.listdir(par.ruta_fuente_digitadas_up) if archivo_modificado_hoy(os.path.join(par.ruta_fuente_digitadas_up, f))]
        archivos_actualizados = archivos
        #df_digitadas_up_historico = consultarDigitadasUpHistorico()


        if par.nombre_archivo_digitadas_up in archivos_actualizados:  
            id_ejecucion = generate_uuid().upper()
            id_ejecucion_en_curso = id_ejecucion
            base_csv_digitadas_up = importarDigitadasUpCsv(par.ruta_fuente_digitadas_up, par.nombre_archivo_digitadas_up)
            df_digitadas_up_nuevo = limpiezaCamposDigitadasUp(base_csv_digitadas_up,fecha_inicio_date,id_ejecucion)
            #df_digitadas_up_nuevo = pd.merge(df_limpieza_campos_digitadas_up, df_digitadas_up_historico, left_on='ot', right_on= ['ot'], how='outer', indicator=True, suffixes=('_digitadas_up', '_historico')).query('_merge == "left_only"')
            #columnas_seleccionadas = [par.mapeo_columnas_digitadas_up[col] for col in par.mapeo_columnas_digitadas_up.keys() if par.mapeo_columnas_digitadas_up[col] in df_digitadas_up_nuevo.columns]
            #df_digitadas_up_nuevo = df_digitadas_up_nuevo[columnas_seleccionadas]
            #df_digitadas_up_nuevo.columns = df_digitadas_up_nuevo.columns.str.replace('_digitadas_up', '')
            fuentes.append(par.nombre_archivo_digitadas_up)
            registros = len(df_digitadas_up_nuevo)
            cantidad_registros.append(registros)
            df_digitadas_up_nuevo = df_digitadas_up_nuevo.reset_index(drop=True)
            
            if registros > 0:
                df_resumen = cargueResumen(id_ejecucion, fecha_inicio_date, par.nombre_archivo_digitadas_up,registros,par.destino_digitadas_up,1)
                cargueDatosBD(df_digitadas_up_nuevo)
