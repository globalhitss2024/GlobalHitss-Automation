...
* OBJETIVO: Data Cruda base instaladas 999                                            *                                                                            *
* TABLA DE INGESTA POSTGRESQL: tbl_dados_crudos_instaladas_999                        *
...
fuentes = []
cantidad_registros = []
destino = [par.destino_instaladas_999]
estado = []
funcion_error = []
descripcion_error = []
...
def cargueResumen(id_ejecucion, fecha_inicio_date,fuentes,cantidad_registros,destino,estado):
    ...
    try:
        ...
        fuentes.append(par.nombre_archivo_instaladas_999)
...
def insertarErroresDB():
    ...
    try:
        ...
        cargueResumen(id_ejecucion_en_curso, fecha_inicio_tr,par.nombre_archivo_instaladas_999,0,par.destino_instaladas_999,2) 
        salidaLogMonitoreo()
...
# %%
def importarInstaladas999Excel(ruta, nombre_archivo, hoja_calculo):
    """
    ...
    try:
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(importarInstaladas999Excel.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def consultarInstaladas999Historico():
    """
    ...
    try:
        ...
        sql_consulta = """
        SELECT *
        FROM fuentes_cruda.tb_datos_crudos_instaladas_999
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(consultarInstaladas999Historico.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
# %%
def limpiezaCamposInstaladas999(base, fecha_inicio_date, id_ejecucion):
    """
    ...
    try:
        ...
        df_base.fillna({
            'numero_contrato': 'NO REGISTRA',
            'nombres': 'NO REGISTRA',
            'tipo_documento': 'NO REGISTRA',
            'numero_documento': 'NO REGISTRA',
            'numero_telefono_1': 'NO REGISTRA',
            'numero_telefono_2': 'NO REGISTRA',
            'numero_calle': 'NO REGISTRA',
            'direccion_residencia': 'NO REGISTRA',
            ...
            'estado': 'NO REGISTRA',
            'tarifa': 'NO REGISTRA',
            'campana_1': 'NO REGISTRA',
            'campana_2': 'NO REGISTRA',
            'campana_3': 'NO REGISTRA',
            'nombre_servicio': 'NO REGISTRA',
            'descripcion_servicio': 'NO REGISTRA',
            'numero_dealer': 'NO REGISTRA',
            'nombre_dealer': 'NO REGISTRA',
            'grupo_dealer': 'NO REGISTRA',
            'coordinador': 'NO REGISTRA',
            'tipo_servicio': 'NO REGISTRA',
            'wo_kind': 'NO REGISTRA',
            'razon_servicio': 'NO REGISTRA',
            'nodo': 'NO REGISTRA',
            'nombre_nodo': 'NO REGISTRA',
            'wo_creador': 'NO REGISTRA',
            'tipo_venta': 'NO REGISTRA',
            'val_dif_service': 0,
            'valor_servicio': 0,
            'renta_wo_anterior': 0,
            'renta_wo_actual': 0,
            'diferencia_renta': 0,
            'numero_lineas_suscriptor': 0,
            'numero_servicios': 0,
            'origen_datos': 'NO REGISTRA',
            'estrato': 'NO REGISTRA',
            ...
            'email': 'NO REGISTRA',
            'adicional_inf1': 'NO REGISTRA',
            'especialista': 'NO REGISTRA',
            'area_gcia_vtas': 'NO REGISTRA',
            'zona_gcia_vtas': 'NO REGISTRA',
            'canal': 'NO REGISTRA',
            'aliado': 'NO REGISTRA',
            'poblacion': 'NO REGISTRA',
            'area_venta': 'NO REGISTRA',
            'zona_venta': 'NO REGISTRA',
            'distrito': 'NO REGISTRA',
            'tipo_red': 'NO REGISTRA',
            'grupo_venta': 'NO REGISTRA'
        }, inplace=True)

        df_historico = consultarInstaladas999Historico()
        ...
        df_nuevos['fecha_digitacion'] = df_nuevos['fecha_digitacion'].fillna(datetime(1899, 12, 30))
        df_nuevos['fecha_completado'] = df_nuevos['fecha_completado'].fillna(datetime(1899, 12, 30))
        ...
        df_nuevos['fuente'] = 'INSTALADAS 999 CORREOS'
        ...
        cantidad_registros.append(0)
        estado.append(2)
        funcion_error.append(limpiezaCamposInstaladas999.__name__)
        descripcion_error.append(str(e)[:100])
        insertarErroresDB()
        salidaLogMonitoreo()
...
def cargueDatosBD(df_final):
    ...
    try:
        ...
        nombre_tabla = 'tb_datos_crudos_instaladas_999'
...
def configurarLogging():
    ...
    log_file = os.path.join(log_directory, "cargue_datos_crudos_instaladas_999.log")
...
if __name__ == "__main__":
    ...
    try:
        ...
        archivos = [f for f in os.listdir(par.ruta_fuente_instaladas_999) if archivo_modificado_hoy(os.path.join(par.ruta_fuente_instaladas_999, f))]
        archivos_actualizados = archivos
        #df_instaladas_999_historico = consultarInstaladas999Historico()


        if par.nombre_archivo_instaladas_999 in archivos_actualizados:  
            id_ejecucion = generate_uuid().upper()
            id_ejecucion_en_curso = id_ejecucion
            base_excel_instaladas_999 = importarInstaladas999Excel(par.ruta_fuente_instaladas_999, par.nombre_archivo_instaladas_999, par.nombre_hoja_instaladas_999)
            df_instaladas_999_nuevo = limpiezaCamposInstaladas999(base_excel_instaladas_999,fecha_inicio_date,id_ejecucion)
            #df_instaladas_999_nuevo = pd.merge(df_limpieza_campos_instaladas_999, df_instaladas_999_historico, left_on='ot', right_on= ['ot'], how='outer', indicator=True, suffixes=('_instaladas_999', '_historico')).query('_merge == "left_only"')
            #columnas_seleccionadas = [par.mapeo_columnas_instaladas_999[col] for col in par.mapeo_columnas_instaladas_999.keys() if par.mapeo_columnas_instaladas_999[col] in df_instaladas_999_nuevo.columns]
            #df_instaladas_999_nuevo = df_instaladas_999_nuevo[columnas_seleccionadas]
            #df_instaladas_999_nuevo.columns = df_instaladas_999_nuevo.columns.str.replace('_instaladas_999', '')
            fuentes.append(par.nombre_archivo_instaladas_999)
            registros = len(df_instaladas_999_nuevo)
            cantidad_registros.append(registros)
            
            if registros > 0:
                df_resumen = cargueResumen(id_ejecucion, fecha_inicio_date, par.nombre_archivo_instaladas_999,registros,par.destino_instaladas_999,1)
                cargueDatosBD(df_instaladas_999_nuevo)
        ...
        else:   #Modificacion para solucion de error name (duracion_proceso' is not defined): Mario Puello 21/01/2025
            duracion_proceso = 0
